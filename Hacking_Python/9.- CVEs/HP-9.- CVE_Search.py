...
def search_cve(link, startIndex="0"):
    ...
    if response.status_code == 200:
        ...
        for result in soup.find_all(
                lambda tag: tag.name == 'a' and tag.get('data-testid', '').startswith('vuln-detail-link-')):
...
def main():
    ...
    if (cve_search_str == ""):
        ...
        remainder = 1 if (
                    matching_records_count % displaying_count_through != 0 and matching_records_count > displaying_count_through) else 0
